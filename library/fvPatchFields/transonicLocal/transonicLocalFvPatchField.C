...
#include "transonicLocalFvPatchField.H"
...
Foam::transonicLocalFvPatchScalarField
::transonicLocalFvPatchScalarField
(
    const fvPatch& p,
...
::transonicLocalFvPatchScalarField
(
    const transonicLocalFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
void Foam::transonicLocalFvPatchScalarField::updateCoeffs()
{
    if (this->updated())
    {
        return;
    ...
    else
    {
        if (this->size() > 0) // To  stop other processors from calculation
        {
            ...
            this->valueFraction() = Zero + neg(Mc - 1.0);
        }
    }
...
void Foam::transonicLocalFvPatchScalarField::write(Ostream& os) const
{
