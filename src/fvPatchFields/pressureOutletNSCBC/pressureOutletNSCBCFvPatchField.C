...
void Foam::pressureOutletNSCBCFvPatchField<Type>::updateCoeffs()
{
    ...
    const fvPatchScalarField& alphagas =
            this->patch().template lookupPatchField<volScalarField, scalar>("alpha.gas");

    const fvPatchVectorField& Ugas =
            this->patch().template lookupPatchField<volVectorField, vector>("U.gas");

    const fvPatchScalarField& alphaparticles =
            this->patch().template lookupPatchField<volScalarField, scalar>("alpha.particles");

    const fvPatchVectorField& Uparticles =
            this->patch().template lookupPatchField<volVectorField, vector>("U.particles");

    const vectorField UpPatch(alphagas*Ugas + alphaparticles*Uparticles);
    const vectorField UpiF(alphagas.patchInternalField()*Ugas.patchInternalField()
            + alphaparticles.patchInternalField()*Uparticles.patchInternalField());

    const vectorField UpSnGrad(this->patch().deltaCoeffs()*(UpPatch - UpiF));
    ...
    if (lInf_ > 0)
    {
	    ...
	    if
		    ...
		    )
		    {
			    ...
			    // ref-B.2.3
			    this->refGrad() = - 1.0 * rhop * cP * (this->patch().nf() & UpSnGrad) * fieldInf_;
		    }
	    else if (ddtScheme == fv::backwardDdtScheme<scalar>::typeName)
	    {
		    // Calculate the field relaxation coefficient k (See A2.2.2)
